...
_BIOS_:NAME::
	...
	;RAM:E4A4 C9                       ret
	dbpatch	:PPI2C_BIOS_INIT:+1,0x49	,0xc9
	...
	dbpatch	:DRVREG_A: 	,0x01	,0x00
	dbpatch	:DRVREG_B: 	,0x02	,0x00
	dbpatch	:DRVREG_C: 	,0x04	,0x01
	dbpatch	:DRVREG_D: 	,0x08	,0x02
	...
	dwstore	res_DRIVE_TAB+0*2	,:DRVREG_A:
	dwstore	res_DRIVE_TAB+1*2	,:DRVREG_B:
	dwstore	res_DRIVE_TAB+2*2	,:DRVREG_C:
	dwstore	res_DRIVE_TAB+3*2	,:DRVREG_D:
	...
	dwpatch 0xDA1B+1 		,:SELDSK_ADDR:	,res_seldsk
	dwstore	old_seld_dsk+1	,:SELDSK_ADDR:
	...
	dbpatch	:SELDSK_STORE_DRV:	,0x77	,0x00
	...
	dwstore	r_p_SELDSKDRIVER+1	,:CURRENT_DISK_PTR:
	dwstore	r_p_SELDSKDRIVEW+1	,:CURRENT_DISK_PTR:
	...
	dwpatch	0xDA27+1	,:BIOS_READ:	,res_READ
	dwpatch	0xDA2A+1	,:BIOS_WRITE:	,res_WRITE
	...
	dwpatch	:WBOOT_READ_CALL:+1	,:BIOS_READ:	,res_READ
	...
	dwpatch	:SELDSK_GETINFO_CALL:+1	,:BIOS_GETINFO:	,res_GETINFO

	;BIOS_xxxx--------------------------------------------------------------
	;сохраняем в "резидет" адреса оригинальных функций
	;они вызываются если идёт обращение к физичесским дискам

	dwstore	_old_read+1			,:BIOS_READ:
	dwstore	_old_write+1		,:BIOS_WRITE:
	dwstore	_old_getinfo+1		,:BIOS_GETINFO:
	...
	dbpatch	:GETINFO_POSTDTOM_ADDR:		,0x21	,0x21
	dwpatch	:GETINFO_POSTDTOM_ADDR:+1 	,:GETINFO_POSTDTOM_CHKW: ,:GETINFO_POSTDTOM_CHKW:

	;и устанавливаем в наш "резидент" переход на эту точку.
	
	dwstore	_old_getinfo_chkdo+1	,:GETINFO_POSTDTOM_ADDR:
	...
	dwstore	r_p_DSK1+1	,:BIOS_VAR_DSK: 	;read
	dwstore	r_p_TRK1+1	,:BIOS_VAR_TRK: 	;read
	dwstore	r_p_SEC1+1	,:BIOS_VAR_SEC: 	;read
	dwstore	r_p_DMA1+1	,:BIOS_VAR_DMA: 	;read

	dwstore	r_p_DSK2+1	,:BIOS_VAR_DSK: 	;write
	dwstore	r_p_TRK2+1	,:BIOS_VAR_TRK: 	;write
	dwstore	r_p_SEC2+1	,:BIOS_VAR_SEC: 	;write
	dwstore	r_p_DMA2+1	,:BIOS_VAR_DMA: 	;write

	dwstore	r_p_DSK3+1	,:BIOS_VAR_DSK: 	;readinfo
	...
	dwpatch	:VINTKBDADDR: ,:VINTKBDVALUE:	,:KBD_HOOK_PROC:
	dwstore	old_hook+1	,:VINTKBDVALUE:
...
h_cpm={
	...
	:SELDSK_STORE_DRV		=> "0xE70A",
	:SELDSK_ADDR			=> "0xDC8D",
	:CURRENT_DISK_PTR   	=> "0xE06E",

	:BIOS_READ 				=> "0xDCEB",
	:BIOS_WRITE 			=> "0xDEAF",

	:SELDSK_GETINFO_CALL	=> "0xdcbf",
	:BIOS_GETINFO 			=> "0xE648",

	:WBOOT_READ_CALL 		=> "0xDB84",

	:GETINFO_POSTDTOM_ADDR 	=> "0xE67F",
	:GETINFO_POSTDTOM_CHKW 	=> "0xE6AF",
...
class PATHCH_GEN
	def initialize(type,h_values)
		txt=File.read("./patch_template_#{type}.asm")
		@h_values=h_values
		
		@checkers=@h_values[:CUSTOM_CHECKERS]
		@checkers||=[]
		# @h_values.delete(:CUSTOM_CHECKERS)
		@h_values[:CUSTOM_CHECKERS]=create_checkers()
		ap @checkers

		@array=filter_comments(txt,h_values[:NAME] == '21_89___wiza1') 
	end
	...
	def create_checkers
		...
		@checkers.each do |chkrec|
			...
			type=nil
			case value
			when /^0x[0-9A-Fa-f]{2}$/i
				type="db"
			when /^0x[0-9A-Fa-f]{4}$/i
				type="dw"
			else
	...
	def filter_comments(txt,do_no_remove_comment)
		array=txt.split("\n")
		# array=array.each_with_index.map{|x,i| x+"|#{i}"}
		unless do_no_remove_comment
...
# ap ENV.values_at("LC_ALL","LC_CTYPE","LANG").compact.first
# ap $stdout.encoding
z=PATHCH_GEN.new("cpm",h_cpm)
puts z.apply